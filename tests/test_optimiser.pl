...
:- begin_tests(loop_dependency_schedule).
...
test(global_invariant_dep_hoist) :-
    BodyIn = [
        hoisted(expensive_dependency(seed)),
        for(I, 1, 3, [expensive_dependency(seed), use(I)])
    ],
    schedule_dependencies(BodyIn, [], BodyOut, Report),
    assertion(BodyOut == [
        hoisted(global, expensive_dependency(seed)),
        for(I, 1, 3, [use(I)])
    ]),
    assertion(Report == [
        loop_hoisted_count(0),
        global_hoisted_count(1),
        left_in_loop_count(0)
    ]).

test(outer_loop_scoped_hoist) :-
    BodyIn = for(I, 1, 3, [
        hoisted(expensive_dependency(calc(I))),
        for(J, 1, 2, [expensive_dependency(calc(I)), use(J)])
    ]),
    schedule_dependencies(BodyIn, [], BodyOut, Report),
    assertion(BodyOut == for(I, 1, 3, [
        hoisted(loop(loop_1), expensive_dependency(calc(I))),
        for(J, 1, 2, [use(J)])
    ])),
    assertion(Report == [
        loop_hoisted_count(1),
        global_hoisted_count(0),
        left_in_loop_count(0)
    ]).

test(inner_loop_local_retention) :-
    BodyIn = for(I, 1, 3, [
        hoisted(expensive_dependency(calc(I))),
        expensive_dependency(calc(I)),
        use(I)
    ]),
    schedule_dependencies(BodyIn, [], BodyOut, Report),
    assertion(BodyOut == for(I, 1, 3, [
        hoisted(local, expensive_dependency(calc(I))),
        expensive_dependency(calc(I)),
        use(I)
    ])),
    assertion(Report == [
        loop_hoisted_count(0),
        global_hoisted_count(0),
        left_in_loop_count(1)
    ]).

test(safety_blocks_unsafe_goal_hoist_without_experimental_mode, [cleanup(set_experimental_mode(false))]) :-
    set_experimental_mode(false),
    BodyIn = [
        hoisted(write(seed)),
        for(I, 1, 3, [write(seed), use(I)])
    ],
    schedule_dependencies(BodyIn, [], BodyOut, Report),
    assertion(BodyOut == [
        hoisted(local, write(seed)),
        for(I, 1, 3, [write(seed), use(I)])
    ]),
    assertion(Report == [
        loop_hoisted_count(0),
...
test(report_counters_are_present_in_optimise_program_report, [cleanup(set_experimental_mode(false))]) :-
    set_experimental_mode(false),
    ProgramIR = [
        ir_clause(c1, loop_global(N), [
            hoisted(expensive_dependency(seed)),
            for(I, 1, N, [expensive_dependency(seed), use(I)])
        ], [])
    ],
    optimise_program(ProgramIR, _OptimisedIR, optimisation_report(Report)),
    assertion(member(loop_hoisted_count(0), Report)),
    assertion(member(global_hoisted_count(1), Report)),
    assertion(member(left_in_loop_count(0), Report)).

test(reporter_renders_stage19_lines) :-
    report_lines(optimisation_report([
        loop_hoisted_count(1),
